...
read_idb_.py. Replaces dump_tsys_ext.py. Old-style UDB files, with the
tsys variables, these are now replaced byt files that have the same
variables as IDB files, xsampler and ysampler'''
#jmm, 2017-02-03
...
def avXdata(x, nsec=60):
    ...
    outx = ma.masked_array(outx0, mask = omask)
    nantsnf = np.size(x['px'][:,0])
    outpx = np.zeros((nantsnf, ntnew), dtype=np.float32)
    outpy = np.zeros((nantsnf, ntnew), dtype=np.float32)
...
#        print ssj
        if len(ssj) > 0:
            ...
            pxj = x['px'][:,ssj]
            outpx[:,j]=np.average(pxj, axis=1)
            pyj = x['py'][:,ssj]
            outpy[:,j]=np.average(pyj, axis=1)
        #endif
    #endfor
    ...
    out = {'x':outx,'uvw':uvwarray,'time':tnew,'px':outpx,'py':outpy,'i0':x['i0'],'j0':x['j0'],'lst':lstarray,'pol':x['pol'],'delay':delayarray,'ut':utarray, 'file0':x['file0']}
    return out
#END of avXdata

def udbfile_recreate2(ufile_in, ufilename, nsec=60):
    '''Read in a UDB dataset average in time and write out the file. Uses
    avXdata or readXdata, ufile_in is the input filename (needed for
    source, scan, etc...), ufilename is the output filename.
    '''

    if len(ufile_in) == 0:
        print 'udbfile_recreate2: No file input'
        return []
    #endif

    # Read in the file, average the data if nsec != 1
    x = readXdata(ufile_in)
    if len(x) == 0:
        print 'udbfile_recreate2: Bad file input'
        return []
    #endif
    if nsec != 1:
    ...
    nrv_varlist_string = ['telescop', 'project', 'operator', 'version', 'source', 'scanid', 'proj', 'antlist', 'obstype']
    ...
    #dtsec = round((y['time'][1]-y['time'][0])*(24.0*3600.0))
    dtsec = 60.0 #temporary
    ...
    for j in range(ntimes):
        ...
        utj = y['ut'][j]
        lstj = y['lst'][j]
        #odd things happen to xtsys, ytsys
        pxj = np.zeros(nants*nf, dtype = np.float32)
        pyj = np.zeros(nants*nf, dtype = np.float32)
        for k in range(nants*nf):
            pxj[k] = y['px'][k,j]
            pyj[k] = y['py'][k,j]
        #endfor
        dj = y['delay'][:,j]
    ...
    del(uv) #done
    return ufilename, x, y

#End of udbfile_recreate2
...
def concatXdata(x0, x):
    ...
    #done
    out = {'x':outx,'uvw':uvwarray,'time':timearray,'px':outpx,'py':outpy,'i0':i0array,'j0':j0array,'lst':lstarray,'pol':polarr,'delay':delayarray,'ut':utarray,'file0':x0['file0']}
    return out
#end of concatXdata
