...
def process_date(date='2018-01-01'):
    '''Process one day, passed in as a datetime string, yyyy-mm-dd'''
    t = Time(date)
    out = allday_udb(t=t, savfig=False)   # Process time t
    return out
...
# easiest to test fits output without saving the figure
    ...
    if not t2 is None:
        blah = allday_udb(t=t2, savfig=False)   # Process time t2
