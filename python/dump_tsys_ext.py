...
#    print version
    ...
    if version == "1.0":
        ...
        tpwr = np.array((xtsys[:,:,:,0],ytsys[:,:,:,0]))    #order is npol, nt, nf, nants
        ...
        tpwr2 = np.array((xtsys[:,:,:,1],ytsys[:,:,:,1]))
        tpwr2 = np.swapaxes(tpwr2,1,3)
        tpwr2 = np.swapaxes(tpwr2,0,1)
        nsamp = np.array((xtsys[:,:,:,2],ytsys[:,:,:,2]))
...
#Header information
    ...
    prihdr.set('OBJ_ID', strip_non_printable(xdat['source']), 'Object ID')
    prihdr.set('SCAN_ID', strip_non_printable(xdat['scanid']), 'Scan ID for this dataset')
    prihdr.set('PROJECT_', strip_non_printable(xdat['proj']), 'EOVSA Project ID')
    ...
    prihdr.set('ANTENNA', strip_non_printable(xdat['antennalist']), 'Used antennae')
...
#Header information, strip last character from strings
    ...
    scan_id = strip_non_printable(xdat['scanid'])
    proj_id = strip_non_printable(xdat['proj'])
    ant_list = strip_non_printable(xdat['antennalist'])
