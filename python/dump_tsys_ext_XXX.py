...
def rd_miriad_tsys_file(filelist):
    ...
    if version == "1.0":
        ...
        sfreq = sfreq[good]
        sdf = sdf[good]
        return {'source':src, 'scanid':scanid, 'proj':proj, 'sfreq':sfreq, 'sdf':sdf, 'ut_mjd':utd, 'tsys':tsys, 'file0':filelist[0], 'antennalist':antennalist, 'nants':nants, 'version':version}
    else:
        ...
        sfreq = sfreq[good]
        sdf = sdf[good]
        return {'source':src, 'scanid':scanid, 'proj':proj, 'sfreq':sfreq, 'sdf':sdf, 'ut_mjd':utd, 'tpwr':tpwr, 'tpwr2':tpwr2, 'nsamp':nsamp, 'file0':filelist[0], 'antennalist':antennalist, 'nants':nants, 'version':version}
    #endif
...
#handle xtsys, ytsys
                ...
                ok = np.where(xts != 0)
                xts_flag[ok] = 1.0
                xts_new = np.zeros([na, nants], dtype=np.float32) #recall na is the reduced nfreq
                for i in range(nants):
                    for j in range(na):
                        xtmp = xts[a[j]:b[j], i] #subarray for this freq band
                        xtmp_flag = xts_flag[a[j]:b[j], i]
                        okij = np.where(xtmp_flag != 0.0)
                        if len(okij[0] > 0):
                            xts_new[j, i] = np.sum(xtmp[okij])/np.sum(xtmp_flag[okij])
                        #endif
                    #endfor
                #endfor
            ...
            #Contract the data variable now
            uvout['pol'] = uv['pol']
            uvout['ut'] = uv['ut']
            dataout = data[:na]
...
#            for j in range(na):
#                dtmp = data[a[j]:b[j]]
#                dtmp_flag = dflag[a[j]:b[j]]
#                okij = np.where(dtmp_flag != 0.0)
#                if len(okij[0] > 0):
#                    dataout[j] = np.sum(dtmp[okij])/np.sum(dtmp_flag[okij])
#                else:
#                    dataout[j] = 0+0j
                #endif
            #endif
